...
export default function CreateRoomPage() {
  ...
  useEffect(() => {
    Promise.all([apiFetch<any>("/rooms/villas"), apiFetch<any>("/rooms/room-types"), apiFetch<any>("/rooms/amenities")]).then(
      ([vRes, rtRes, aRes]) => { setVillas(vRes.data); setRoomTypes(rtRes.data); setAmenities(aRes.data); }
    );
  }, []);
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold">Tambah Kamar</h1><p className="text-muted-foreground">Tambahkan kamar baru</p></div>
      </div>
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-2 pt-4"><Button type="submit" disabled={loading}>{loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Simpan</Button><Link href="/dashboard/rooms"><Button type="button" variant="outline">Batal</Button></Link></div>
          </form>
        </CardContent>
      </Card>
    </div>
